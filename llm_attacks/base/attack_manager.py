...
class AttackPrompt(object):
    ...
    def _update_ids(self):#用于更新会话模板中的各个部分的标记 id。根据不同的会话模板，分别更新控制字符串、目标字符串等的标记 id。
    ...
    def generate_str(self, model, gen_config=None):# generate 方法的包装器，用于生成字符串形式的攻击提示的输出。decoder解码之后
...
class PromptManager(object):
    ...
    def __init__(self,
        ...
        *args, **kwargs
        ...
        self._prompts = [
            # 对于每一个goal和其对应的target，生成一个AttackPrompt类
            managers['AP'](
                goal, #为1
                target, #为1
                tokenizer, 
...
class MultiPromptAttack(object):
    ...
    def __init__(self, 
        ...
        *args, **kwargs
        ...
        self.prompts = [
            #对于每一个llm模型，实例化一个PromptManager类来管理prompt
            managers['PM'](# PM 指的是PromptManager
                goals,
                targets,
                worker.tokenizer,
    ...
    def run(self, 
        ...
        #函数 P 定义了一个温度函数，用于模拟退火算法。它接受三个参数 e、e_prime 和 k，
        # 其中 e 表示当前损失，e_prime 表示下一步的损失，k 表示当前步数。
        # 根据模拟退火算法的原理，函数 P 返回一个布尔值，表示是否接受下一步的状态变化。
        ...
        for i in range(n_steps):
            ...
            if stop_on_success:#如果 stop_on_success 为 True，并且当前模型已经满足成功条件，则跳出循环。
...
# 如果需要，记录日志。
            ...
            control, loss = self.step(# 执行一次模型训练，获取控制字符串和损失值。
...
class IndividualPromptAttack(object):
    ...
    def run(self, 
        ...
        """
        Executes the individual prompt attack.执行攻击的函数
        ...
        for i in range(len(self.goals)):#对每一个输入的目标句子攻击，即尝试使其生成prompt 比如，告诉我炸掉学校的方法
            print(f"Goal {i+1}/{len(self.goals)}")
            #用的是 GCGMultiPromptAttack这个类 来对每一个目标 进行实际攻击
            attack = self.managers['MPA'](# MPA对应 GCGMultiPromptAttack，其继承自MultiPromptAttack，每次输入一个goals和target
                self.goals[i:i+1], 
                self.targets[i:i+1],
                self.workers,
                self.control,
            ...
            attack.run(
                ...
                filter_cand=filter_cand,
                verbose=verbose
            )#返回 self.control_str, loss, steps
...
class ModelWorker(object):
    ...
    def start(self):
        #这一行创建了一个Process对象，这是multiprocessing模块中用于表示一个独立进程的类，为每一个模型开一个进程
        self.process = mp.Process(
            target=ModelWorker.run,#指定了这个进程开始执行时应该调用的函数。这里，ModelWorker.run是一个静态方法或类方法，
            # 它定义了进程启动后要执行的具体工作。
            args=(self.model, self.tasks, self.results)
            #提供了ModelWorker.run方法需要的参数。这些参数包括：
            # 一个模型对象self.model，
            # 一个任务队列self.tasks，
            # 和一个用于存放结果的队列self.results。
        )
        self.process.start()
        #这一行启动了之前创建的进程。进程开始运行后，会调用并执行ModelWorker.run方法，
        # 并传入上面指定的参数。
...
def get_workers(params, eval=False):
    ...
    for i in range(len(params.tokenizer_paths)):
        ...
        #用于将文本字符串分解成更小的单元，通常是词、子词（subword）或者标记（token）。
        # 这个过程对于语言模型来说是必要的，因为模型不能直接理解原始文本，而是通过这些分解后的单元来学习和理解语言的结构和意义。
        # tokenizer类不仅负责分词，还负责将这些单词或标记转换为模型能理解的数字ID，
        # 通常还包括添加必要的特殊标记，如序列的开始（BOS）、结束（EOS）标记，填充（PAD）标记等。
        #根据模型的不同要进行调整
    ...
    raw_conv_templates = [
        #得到对应模型的对话模板（User、client对应等），是所有对话模板形成的数组
    ...
    for conv in raw_conv_templates:
        if conv.name == 'zero_shot':
            ...
            conv.sep = '\n'
            #将conv对象的roles属性修改为一个新的元组，该元组由原roles列表中的每个元素前添加字符串'### '形成的新字符串组成。
        elif conv.name == 'llama-2':
            conv.sep2 = conv.sep2.strip()
            #将conv对象的sep2属性修改为其去除首尾空白字符后的值。
        conv_templates.append(conv)
        
    print(f"Loaded {len(conv_templates)} conversation templates")
    workers = [#每一个Modelworker实例由，路径、参数、tokenizer、对话模板、所用device组成
        ModelWorker(
    ...
    if not eval:#如果是评价模型，就可以不启动worker，训练状态则启动
        for worker in workers:
            worker.start()
