...
# 收集所有可能的介质和部位组合
all_media_parts = set((row['unmatch_attribute'].split(' and ')[0], row['unmatch_attribute'].split(' and ')[1]) for row in data_rows)
...
for row in data_rows:
    ...
    possible_replacements = list(all_media_parts - {(row['unmatch_attribute'].split(' and ')[0], row['unmatch_attribute'].split(' and ')[1])})
...
with open('3MAD-28K.csv', mode='w', newline='') as file:
    fieldnames = ['id', 'file_name', 'original_attribute','unmatch_attribute', 'normal_prompt','harmful_prompt', 'policy', 'key_phrases']
